create or replace PACKAGE              "HOSPITALDRUG_PACK_SA" AS
  FUNCTION find_unit_price(p_hospdrugcode VARCHAR2,
                           p_hcode        VARCHAR2,
                           p_date         DATE) RETURN NUMBER;
  FUNCTION find_status_ed(p_hospdrugcode VARCHAR2,
                          p_hcode        VARCHAR2,
                          p_date         DATE,
                          v_current_tmt  VARCHAR2) RETURN VARCHAR2;
  FUNCTION find_status_ed2(p_hospdrugcode VARCHAR2,
                           p_hcode        VARCHAR2,
  ...
  FUNCTION find_tmtid_intx(p_hospdrugcode VARCHAR2,
                           p_hcode        VARCHAR2,
                           p_date         DATE) RETURN VARCHAR2;
  FUNCTION find_hospdrug(p_hospdrugcode VARCHAR2,
                         p_hcode        VARCHAR2,
  ...
  FUNCTION find_hospdrug_withtmt(p_hospdrugcode VARCHAR2,
                                 ...
                                 p_tmtid        VARCHAR2,
                                 p_date         DATE) RETURN hospitaldrug;
  FUNCTION find_tmtid_intx_withtmt(p_hospdrugcode VARCHAR2,
                                   p_hcode        VARCHAR2,
                                   p_tmtid        VARCHAR2,
                                   p_date         DATE) RETURN VARCHAR2;
...
create or replace PACKAGE BODY               HOSPITALDRUG_PACK_SA AS
  ...
  FUNCTION find_status_ed(p_hospdrugcode VARCHAR2,
                          p_hcode        VARCHAR2,
                          p_date         DATE,
                          v_current_tmt  VARCHAR2) RETURN VARCHAR2 AS
  ...
  BEGIN
    FOR rcd IN (SELECT hdrg.tmtid,
      ...
      -- find drug
      BEGIN
        SELECT drg.tmtid, drg.type tmt_type, drg.fsn, drg.dosageform
          INTO v_tmtid, v_tmt_type, v_fsn, v_tmt_dosageform
          FROM tmt_drug drg
...
--                                                  p_date);
      ...
      **/
      ...
      v_hdrug.hosp_genericName := v_drugattr.hosp_genericName;
      v_hdrug.hosp_tradeName   := v_drugattr.hosp_tradeName;
      /**    v_hdrug.unit_price       := v_unit_price; change from drugattr.unit_price --by Suwatchai**/
